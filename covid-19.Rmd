...
library(flexdashboard) ; library(shiny) ; library(tidyverse) ; library(sf) ; library(htmlwidgets) ; library(htmltools) ; library(leaflet) ; library(leaflet.extras) ; library(reactable) ; library(lubridate) ; library(zoo) ; library(scales) ; library(ggtext)
...
# table data
tbl <- select(df, area_name, current_week_rate, previous_week_rate, change_rate)
...
output$chart <- renderPlot({
  ...
  ggplot() +
    ...
    labs(x = NULL, y = NULL,
       ...
       subtitle = paste0("<span style = 'color:#757575;'>", id, ", to ", format(max(cases$date), '%d %B %Y'), "</span>"),
    ...
    theme(plot.margin = unit(rep(0.5, 4), "cm"),
          ...
          plot.title = element_text(colour = "#757575", size = 16, face = "bold"),
...
output$mapUI <- renderUI({
  if (is.null(selected())) {
    div(HTML("Please <strong>select a local authority</strong> from the table to visualise it and neighbouring areas on a map")) 
      ...
      fillCol(flex = c(NA, 1),
              HTML(paste0("<p style = 'color: #757575;font-size: 1.1em;font-weight: bold;margin-left: 15px;'>Confirmed cases during week ending ", format(max(pull(filter(cases, date == max(date)-days(5)),date)), '%d %B'),"</p>")),
              leafletOutput("map")
      )
      }
  })
