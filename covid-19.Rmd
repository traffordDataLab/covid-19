...
```{r}
renderPlotly({
  p <- ggplot(stringency_index,  aes(x = date_value, y = stringency, colour = country, label = country, group = country, text = paste0("<b>", country, "</b><br><b>", date_value, "</b></br>Stringency Index: <b>", round(stringency,0), "</b>"))) +
    geom_hline(yintercept = 0, size = 0.3, colour = "#333333") +
    geom_line(size = 0.8) +
    geom_text(data = stringency_index %>% group_by(country) %>% filter(date_value == max(date_value)) %>% ungroup(), aes(x = date_value, y = stringency), nudge_x = -2.5, nudge_y = 1.4, fontface = "bold", check_overlap = TRUE) +
  ...
  p$x$data[[8]]$hoverinfo <- "none"
  p$x$data[[9]]$hoverinfo <- "none"
  p$x$data[[10]]$hoverinfo <- "none"
  p
