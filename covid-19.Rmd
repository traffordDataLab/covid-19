...
### New confirmed cases in last 7 days
```{r}
cumulative_cases <- la_data %>% 
  mutate(popup = str_c("<strong>", area_name, "</strong><br/><strong>", cum_cases, "</strong> confirmed cases<br/>to ", format(date, "%d %B")) %>% map(HTML))
...
renderLeaflet({
  ...
  addCircleMarkers(data = cumulative_cases, lng = ~long, lat = ~lat, radius = ~sqrt(cum_cases), fillColor = "#57AACB", fillOpacity = 0.8, weight = 1, color = "#FFFFFF", opacity = 1, label = ~popup, labelOptions = labelOptions(style = list("font-weight" = "normal", padding = "3px 8px"), textsize = "13px", direction = "auto")) %>% 
...
> **Notes**: 1) The size of the circles are proportional to the number of confirmed cases over the last seven days in each local authority district. 2) Confirmed cases for Cornwall and Isles of Scilly are combined.<br/>**Source**: [Public Health England](https://coronavirus.data.gov.uk)

### Rate of new confirmed cases in last 7 days
```{r}
infection_rate <- la_data %>%
  mutate(rate = round(cum_cases/population*100000,0),
         popup = str_c("<strong>", area_name, "</strong><br/><strong>", rate, "</strong> cases per 100,000 residents<br/>to ", format(date, "%d %B")) %>% map(HTML))
...
> **Notes**: The rate of cases is calculated by dividing the cumulative number of cases over the last seven days by the mid-2018 estimate of the resident population of each local authority multiplied by 100,000.<br/>**Source**: [Public Health England](https://coronavirus.data.gov.uk) | [Nomis / ONS](https://www.nomisweb.co.uk/datasets/pestsyoala)
