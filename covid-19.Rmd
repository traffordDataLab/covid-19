...
# map data
sf <- msoa %>% 
  left_join(msoa_cases, by = "msoa21cd") %>% 
  mutate(n = if_else(is.na(n),0,n)) %>%
  mutate(popup = if_else(n == 0,
                        str_c("<strong>", msoa21hclnm, "</strong><br/><em>", lad22nm, "</em><br/>0-2 cases") %>% map(HTML),
                        str_c("<strong>", msoa21hclnm, "</strong><br/><em>", lad22nm, "</em><br/><strong>", n, "</strong> cases<br/><strong>", comma(round(rate, 0), accuracy = 1), "</strong> per 100,000") %>% map(HTML)))
```
...
```{r}
fillCol(flex = c(NA, 1),
  HTML("<p style = 'color: #707070;font-size: 1.1em;font-weight: bold;margin-left: 15px;'>Weekly confirmed cases per 100,000 (rounded)</p>"),
...
output$chart <- renderPlot({
  ...
  ggplot() +
    geom_hline(yintercept = 0, linewidth = 0.3, colour = "#333333") +
    geom_col(data = selected_cases, aes(x = date, y = new_cases), fill = "#39809E", colour = NA, width = 0.8, alpha = 0.6) +
    geom_line(data = selected_cases, aes(x = date, y = ma_cases, colour = "ma_cases"), linewidth = 1, alpha = 1) +
    scale_colour_manual(values = c("ma_cases" = "#39809E"), name = NULL, labels = "7-day average") +
    scale_x_date(breaks = c(min(selected_cases$date), max(selected_cases$date)), date_labels = "%d %b '%y") +
...
zoom_to <- reactive({
  id <- tbl[selected(), ]$area_name
  filter(sf, lad22nm == id) 
  })

output$map <- renderLeaflet({
  
    leaflet(data = zoom_to()) %>%
    addTiles(urlTemplate = "https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png", attribution = '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors &copy; <a href="https://carto.com/attributions">CARTO</a> | <a href="https://www.ons.gov.uk/methodology/geography/licences">Contains OS data © Crown copyright and database right (2021)</a>') %>%
