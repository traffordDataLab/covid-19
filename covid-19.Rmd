...
output$cases_plot <- renderPlotly({
  p <- ggplot(daily_cases, aes(x = DateVal, y = CumCases, group = 1, text = paste0("<b>", comma(CumCases), "</b> cases<br>", format(DateVal, "%d %B")))) +
    ...
    p <- p + scale_y_continuous(expand = c(0.1,0.1), labels = comma) + geom_hline(yintercept = 0, size = 0.3, colour = "#333333")
    } else {
      p <- p + scale_y_log10(expand = c(0.1,0.1)) + geom_hline(yintercept = 1, size = 0.3, colour = "#333333")
    }
...
output$deaths_plot <- renderPlotly({
  ...
  if (input$deaths_scale == "Linear") {
    p <- p + geom_hline(yintercept = 0, size = 0.3, colour = "#333333") + scale_y_continuous(expand = c(0.1,0.1), labels = comma)
    } else {
      p <- p + geom_hline(yintercept = 01, size = 0.3, colour = "#333333") + scale_y_log10(expand = c(0.005, 0.005))
    }
