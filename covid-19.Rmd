...
sf <- ltla %>% 
  ...
  mutate(popup = str_c("<strong>", area_name, "</strong><br/>", paste(format(max(cases$date)-days(8),'%d %b'),"-", format(max(cases$date)-days(2),'%d %b')),":<strong> ", current_week, "</strong><br/>", paste(format(max(cases$date)-days(15),'%d %b'),"-", format(max(cases$date)-days(9),'%d %b')), ":<strong> ", previous_week, "</strong><br/>Change:<strong> ", change, "</strong>") %>% map(HTML),
...
output$table <- renderReactable({
  ...
  reactable(tbl, 
            ...
            if (tbl[index, "current_week_rate"] >= 50) {
              list(background = "rgba(220,24,65,0.1)")
              } else if (tbl[index, "current_week_rate"] >= 25) {
...
output$mapUI <- renderUI({
  if (is.null(selected())) {
    div(HTML("Please <strong>select a local authority</strong> from the table to visualise it and neighbouring areas on a map")) 
      ...
      fillCol(flex = c(NA, 1),
              HTML(paste0("<p style = 'color: #707070;font-size: 1.1em;font-weight: bold;margin-left: 15px;'>Map of confirmed cases during week ending ", format(max(pull(filter(cases, date == max(date)-days(2)),date)), '%d %B %Y'),"</p><br/>")),
              leafletOutput("map")
      )
      }
  })
