...
library(flexdashboard) ; library(shiny) ; library(tidyverse) ; library(httr) ; library(readxl) ;  library(sf) ;  library(htmlwidgets) ; library(htmltools) ; library(leaflet) ; library(leaflet.extras) ; library(DT) ; library(scales) ; library(classInt) ; library(plotly) ; library(DT) 
...
output$cases_plot <- renderPlotly({
  p <- ggplot(daily_cases, aes(x = DateVal, y = CumCases, group = 1, text = paste0("<b>", comma(CumCases), "</b> cases<br>", format(DateVal, "%d %B")))) +
    ...
    p <- p + geom_hline(yintercept = 0, size = 0.3, colour = "#333333") + scale_y_continuous(expand = c(0.005, 0.005), labels = comma)
      ...
      p <- p + geom_hline(yintercept = 1, size = 0.3, colour = "#333333") + scale_y_log10(expand = c(0.005, 0.005))
    }
...
renderDT({
  daily_cases %>% 
    select(1:3) %>% 
    mutate(CMODateCount = comma(CMODateCount),
           CumCases = comma(CumCases)) %>% 
    ...
    datatable(extensions = 'Scroller',
              ...
              colnames = c("Date", "New cases", "Total cases"))
})
...
### Confirmed deaths by date
```{r}
renderDT({
  daily_deaths %>% 
    arrange(desc(Date)) %>% 
        mutate(NewDeaths = comma(NewDeaths),
               CumDeaths = comma(CumDeaths)) %>% 
    datatable(extensions = 'Scroller',
              style = 'bootstrap', 
              class = 'table-bordered',
              fillContainer = FALSE,
              ...
              options = list(
                ...
                scroller = TRUE,
                columnDefs = list(list(className = 'dt-left', targets = "_all"))),
              colnames = c("Date", "New deaths", "Total deaths"))
})
```
...
<strong>Please note</strong>
  <ul>
    ...
    <li>The date refers to the date of <em>reporting</em> not necessarily the date that confirmed cases or deaths occurred on</li>
  </ul>
