...
library(flexdashboard) ; library(shiny) ; library(tidyverse) ; library(httr) ; library(htmlwidgets) ; library(htmltools) ; library(zoo) ; library(scales) ; library(plotly) ; library(classInt) ; library(leaflet) ; library(leaflet.extras)
...
> **Notes**: The rate of cases is calculated by dividing the cumulative number of cases over the last seven days by the mid-2019 estimate of the resident population of each local authority multiplied by 100,000.<br/>**Source**: [Public Health England](https://coronavirus.data.gov.uk) | [ONS](https://www.ons.gov.uk/peoplepopulationandcommunity/populationandmigration/populationestimates/datasets/populationestimatesforukenglandandwalesscotlandandnorthernireland)
...
```{r}
renderPlotly({
  p <- ggplot(country_data, aes(x = days, y = total_deaths, color = countriesAndTerritories, label = countriesAndTerritories, group = countriesAndTerritories, text = paste0("<b>", countriesAndTerritories, "</b><br><b>", days, "</b> days", "</br><b>", comma(total_deaths, accuracy = 1), "</b> deaths"))) + 
  ...
  scale_y_continuous(trans = "log10", breaks = c(100, 200, 500, 1000, 2000, 5000, 10000, 20000, 50000, 100000), limits = c(100,100000), expand = expansion(add = c(0,0.1)), labels = comma_format(accuracy = 1)) +
...
renderPlot({
  ggplot(stringency_index,  aes(x = Date, y = Stringency, colour = Country, label = Country, group = Country)) +
    ...
    geom_line(size = 1) +
    scale_colour_manual(values = c("France" = "#52BCA3",  "Italy" = "#99C945", "Germany" = "#CC61B0","Netherlands" = "#24796C", "Spain" = "#2F8AC4", "Sweden" = "#764E9F", "UK" = "#ED645A")) +
    scale_x_date(expand = c(0.005, 0.4), date_labels = "%b") +
