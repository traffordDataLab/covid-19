  ...
  if (input$cases_scale == "Linear") {
    ...
    ggplotly(p, tooltip = "text") %>%
      ...
      temp <- daily_cases %>% 
        arrange(DateVal) %>% 
        filter(CumCases > 0) %>%
        mutate(days = as.integer(DateVal - min(DateVal)),
               doubling_2_day = 1*(1+((2^(1/2))-1))^(days),
               doubling_3_day = 1*(1+((2^(1/3))-1))^(days),
               doubling_1_week = 1*(1+((2^(1/7))-1))^(days))
      
      p <- ggplot(temp, aes(x = DateVal, y = CumCases, group = 1, text = paste0("<b>", comma(CumCases, accuracy = 1), "</b> cases<br>", format(DateVal, "%d %B")))) +
        ...
        geom_line(aes(x = DateVal, y = doubling_2_day), colour = "#bdbdbd", lty = 3) +
        geom_line(aes(x = DateVal, y = doubling_3_day), colour = "#bdbdbd", lty = 2) +
        geom_line(aes(x = DateVal, y = doubling_1_week), colour = "#bdbdbd", lty = 4) +
        labs(x = NULL, y = NULL, title = NULL, subtitle = NULL, caption = NULL) +
        theme_minimal() +
        theme(plot.margin = unit(rep(0.5, 4), "cm"),
              panel.grid.major.x = element_blank(),
...
<strong>Please note</strong>
  <ul>
    ...
    <li>The date refers to the date of <em>reporting</em> not necessarily the date of confirmed cases or deaths</li>
  </ul>
